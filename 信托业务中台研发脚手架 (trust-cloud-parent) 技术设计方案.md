...
```plain
trust-cloud-parent (Root POM / BOM)
...
<parent>
    ...
    <artifactId>trust-cloud-parent</artifactId>
...
+ **开发预发布版 (DEV)**：
    - **节奏**：随时发布。
    - **场景**：异地团队开发新功能急需底层支持时，使用 `1.1.0-dev`。
    - _注意：生产环境严禁使用开发预发布版。_
+ **RELEASE (正式版)**：
    - **节奏**：**双周一版**（与业务迭代对齐）。
    - **策略**：
